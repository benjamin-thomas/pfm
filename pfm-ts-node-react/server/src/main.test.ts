...
import { serverInit, type Config } from './main';
...
describe('Server E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    server = serverInit(transactionRepo, testConfig);
  ...
  describe('Transactions endpoints', () => {
    ...
    it('should handle invalid transaction id', async () => {
      ...
      // httpDispatch2 correctly returns 400 for parameter validation errors
      expect(response.statusCode).toBe(400);

      const data = JSON.parse(response.data);
      expect(data).toHaveProperty('error');
      expect(data.error).toContain("Decoding error: invalid parameter 'invalid'");
    });
  });
